# =======================================================
# Automatic treatment of ECB SPF gdp data
# =======================================================
all.info.delete.data.G <- NULL
...
yq.gdp <- yq #paste(yq,"_gdp",sep="")
if(first.quarter>1){
  yq.gdp <- yq.gdp[first.quarter:length(yq.gdp)]
}
if(last.quarter<4){
  yq.gdp <- yq.gdp[1:(length(yq.gdp)-(4-last.quarter))]
}

count <- 0
for(spf.file in yq.gdp){
  count <- count+1
  #================================
  # Load data:
  ## Extract the line at which GPD survey starts.
  indic.skip <- grep("GROWTH", readLines(paste("./data/EA/raw/SPF_ECB/",spf.file,".csv",sep="")))
  
  ## Load data
  data <- read.table(paste("./data/EA/raw/SPF_ECB/",spf.file,".csv",sep=""), 
                     sep=",", quote="\"",header=TRUE, skip =indic.skip, na.strings = c("")) %>%
  ...
  data$point.mean.diff <- (abs(data$POINT/data$mean-1)>.5)
  ...
  for(h in unique(data$uniqueHorizon)){ #c(1,2,5)
    ...
    # Append the new row
    all.info.delete.data.G <- rbind(all.info.delete.data.G, new.row)
  ...
  # create database 
  eval(parse(text = gsub(" ","",paste("EA.G.SPF.DISTRI.", spf.file, "<- data %>% 
                                      dplyr::select(date, TARGET_PERIOD, horizon, FCT_SOURCE, POINT, everything()) %>%
                                      rename(Forecaster_ID= FCT_SOURCE)", sep=""))))
  
  ### ROLLING FORECASTS
  eval(parse(text = gsub(" ","",paste("EA.G.SPF.DISTRI.1y.", spf.file, "<- data %>% 
  ...
  eval(parse(text = gsub(" ","",paste("EA.G.SPF.DISTRI.2y.", spf.file, "<- data %>% 
  ...
  ### CALENDAR FORECASTS
  eval(parse(text = gsub(" ","",paste("EA.G.SPF.DISTRI.cy.", spf.file, "<- data %>% 
  ...
  eval(parse(text = gsub(" ","",paste("EA.G.SPF.DISTRI.ny.", spf.file, "<- data %>% 
  ...
  eval(parse(text = gsub(" ","",paste("EA.G.SPF.DISTRI.nny.", spf.file, "<- data %>% 
  ...
  if(data$date[1] %in% date.with.pb.5y){
    ...
    eval(parse(text = gsub(" ","",paste("EA.G.SPF.DISTRI.5y.", spf.file, "<- data %>% 
  ...
  SPF.G.individual.PE.1y.indi <- data %>% filter(horizon==1, integer.horiz==TRUE) %>% dplyr::select(date, FCT_SOURCE, POINT)
  
  if(count==1){
    SPF.G.individual.PE.1y <- distinct(data, date, FCT_SOURCE)
    #SPF.G.individual.PE.1y.all <- SPF.G.individual.PE.1y.indi
    
  } else{
    SPF.G.individual.PE.1y <- rbind(SPF.G.individual.PE.1y,distinct(data, date, FCT_SOURCE))
    #SPF.G.individual.PE.1y.all <- rbind(SPF.G.individual.PE.1y.all,SPF.G.individual.PE.1y.indi)
    
  }
  
  if(count==1){
    
    SPF.G.individual.PE <- data %>% 
    ...
    SPF.G.individual.PE <- rbind(SPF.G.individual.PE, data %>% 
                                   #filter((horizon==1 & integer.horiz==TRUE) | (horizon==2 & integer.horiz==TRUE) | (horizon>4)) %>%
                                   dplyr::select(date, FCT_SOURCE, POINT, horizon, TARGET_PERIOD_OLD, integer.horiz) %>%
                                   rename(date_target=TARGET_PERIOD_OLD,
                                          Forecaster_ID=FCT_SOURCE) #%>%
...
# SPF.G.individual.PE.1y.disagreement <- SPF.G.individual.PE.1y.all %>%
#   group_by(date) %>%
#   mutate(disagreement = sd(POINT, na.rm=T)) %>%
