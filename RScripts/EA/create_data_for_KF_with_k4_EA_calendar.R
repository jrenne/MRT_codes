...
survey.DATA.new.with.param <- survey.DATA.with.param %>%
  pivot_longer(cols = -date, names_to = "var", values_to = "value") %>%
  mutate(
    quarter = quarter(date),
    maturity_q = case_when(
...
observables.surveys.EA <- DATA %>% 
  dplyr::select(date, EA.infl.quarterly.deseasonalized) %>% 
  rename(EA.infl=EA.infl.quarterly.deseasonalized) %>% 
  full_join(survey.DATA.new %>% 
              dplyr::select(date, ends_with(".pe")), by="date")

observables.var.EA <- survey.DATA.new %>% 
  dplyr::select(date, ends_with(".var")) 

observables.k3rd.EA <- survey.DATA.new %>% 
  dplyr::select(date, ends_with(".k3rd")) 

observables.k4th.EA <- survey.DATA.new %>% 
  dplyr::select(date, ends_with(".k4th")) %>%
  mutate(across(-date, ~ if_else(. > 50, 50, .))) #%>% 
#mutate(across(-date, ~ if_else(. < 0, 0, .)))

observables.with.dates.EA <- observables.surveys.EA %>% 
  full_join(observables.var.EA, by="date") %>% 
  full_join(observables.k3rd.EA, by="date") %>%
  full_join(observables.k4th.EA, by="date") %>%
  dplyr::filter(date >= "1999-01-15") %>%
  dplyr::select(-matches("\\b((1|2|3)Q|([9]|1[0-9]|2[0-9])Q)\\b"))
#dplyr::filter(date >= "1997-01-15" & date <= "2022-01-15") 

observables.EA <- observables.with.dates.EA %>% 
  dplyr::select(-date)
...
select.inflation.types <- array(0,c(length(c(H)),1,4))
...
select.inflation.types[1:length(c(H)),1,3] <- 1111 # We have uncertainty measure + 3 and 4 cum


# Make ".EA" matrices:
observables.EA <- as_tibble(observables.EA)
H.EA <- H
select.inflation.types.EA <- select.inflation.types
observables.with.dates.EA <- as_tibble(observables.with.dates.EA)
