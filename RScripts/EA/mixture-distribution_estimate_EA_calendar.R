...
names.all.surveys <- c( ########## Modify
  paste0("EA.SPF.DISTRI.cy.",  yq.infl,".avg"),
  paste0("EA.SPF.DISTRI.ny.",  yq.infl,".avg"),
  paste0("EA.SPF.DISTRI.nny.",  yq.infl[indices_to_keep_nny],".avg"),
  paste0("EA.SPF.DISTRI.5y.",  yq.infl[indices_to_keep],".avg")
)

# The following names are going to be used as prefix in the 
# big DATA base.
short.names.all.surveys <- c( ########## Modify
  rep("SPF.EA.cy.beta",length(yq.infl)),
  rep("SPF.EA.ny.beta",length(yq.infl)),
  rep("SPF.EA.nny.beta",length(yq.infl[indices_to_keep_nny])),
  rep("SPF.EA.5y.beta",length(yq.infl[indices_to_keep]))
)

# Define the horizon to be considered
horizon.spf <- c(str_extract(names.all.surveys, "(?<=EA\\.SPF\\.DISTRI\\.)([^\\.]+)")) # SPF

# Define the horizon to be considered
horizon <- c(horizon.spf) # SPF

survey.DATA <- data.frame(date=unique(SPF.individual.PE.1y$date)) # This is a dataframe that will contain the pe and stdv series (eventually, it will be merged with DATA)
survey.DATA.with.param <- data.frame(date=unique(SPF.individual.PE.1y$date)) # This is a dataframe that will contain the pe and stdv series (eventually, it will be merged with DATA)
...
for(survey in names.all.surveys){
  ...
  print(paste(survey,": working on fitting mixture distributions",sep=""))
  print(dim(survey.DATA))
  ...
  for (col in var_names) {
    cols_x <- paste0(col, ".x")
    cols_y <- paste0(col, ".y")
    survey.DATA <- survey.DATA %>% mutate(!!paste0(col) := coalesce(!!sym(cols_x), !!sym(cols_y)))
  }
  survey.DATA <- survey.DATA %>% dplyr::select(-ends_with(".x"), -ends_with(".y"))
  
  # Merge all data
  survey.DATA.with.param            <- survey.DATA.with.param  %>%
    full_join(survey.stats.with.param, by=c("date"))
  
  ## Extract variables finishing by .x if the column already existed
  var_names <- survey.DATA.with.param %>%
    dplyr::select(ends_with(".x")) %>%
    names() %>%
    sub("\\.x$", "", .)
  ...
  for (col in var_names) {
    cols_x <- paste0(col, ".x")
    cols_y <- paste0(col, ".y")
    survey.DATA.with.param <- survey.DATA.with.param %>% mutate(!!paste0(col) := coalesce(!!sym(cols_x), !!sym(cols_y)))
  }
  survey.DATA.with.param <- survey.DATA.with.param %>% dplyr::select(-ends_with(".x"), -ends_with(".y"))
...
data.to.delete <- all_objects[grep("^EA\\.SPF\\.DISTRI", all_objects)]
# Exclude objects that end with ".avg"
data.to.delete <- data.to.delete[!grepl("\\.avg$", data.to.delete)]
#data.to.delete.1 <- all_objects[grep("^EA.SPF.DISTRI.1y", all_objects)]
#data.to.delete.2 <- all_objects[grep("^EA.SPF.DISTRI.2y", all_objects)]
#data.to.delete.5 <- all_objects[grep("^EA.SPF.DISTRI.5y", all_objects)]
