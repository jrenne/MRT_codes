...
plot(x,y,
     type="l",
     ylim=c(0,0.25),
     col="grey35", xlab="", ylab="", lwd=2,  main="Large (negative) demand shock",
     cex.main = 0.9)
y <- pdf.incompl.beta(x,c(6,2.5,-6,5),-10+0.924,4+0.924)
y[is.na(y)] <- 0
lines(x,y,
      col="black", lwd=2)
abline(v=moments.incompl.beta(c(5,1.5,-6,0),-7+0.775,7+0.775)$Mean, lty=2, col="grey35")
...
legend("topleft",legend=c(expression( mu^(pi)*"=1.9"),
       ...
       cex=1, bty = "n", y.intersp=1.2, text.col="grey35")
legend("topright", legend=c(expression( mu^(Delta*y)*"=1.6"),
...
y[is.na(y)] <- 0
plot(x,y, type="l", ylim=c(0,0.25), col="grey35", xlab="", ylab="", lwd=2, main="Large (negative) supply shock", cex.main = 0.9)
y <- pdf.incompl.beta(x,c(6,2.5,-6,5),-10+0.924,4+0.924)
y[is.na(y)] <- 0
lines(x,y, col="black", lwd=2)
abline(v=moments.incompl.beta(c(1.5,5,0,6),-4.7647+0.775,14-4.7647+0.775)$Mean, lty=2, col="grey35")
abline(v=moments.incompl.beta(c(6,2.5,-6,5),-10+0.924,4+0.924)$Mean, col="black", lty=2)
legend("topleft",legend=c(expression( mu^(pi)*"=1.9"),
                          expression( sigma^(pi)*"=8.7"),
                          expression( mu[3]^(pi)*"=0.5")),
