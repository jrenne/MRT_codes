...
MatrixXd beta_dot_dot_dot_1(MatrixXd gamma, List Model, MatrixXd S_q, MatrixXd S_q_tilde) {
  ...
  // Create diag(mu^4)
  MatrixXd diag_mu_4 = mu.array().cube().matrix().asDiagonal().toDenseMatrix();
  ...
  MatrixXd result_inter = (24*(gamma_Y.transpose()*Theta + gamma_z.transpose()).array().pow(4).matrix()*(diag_mu_4*phi.transpose()) +
    ...
    36*kroneckerProduct(gamma_Y.transpose()*Theta + gamma_z.transpose(),kroneckerProduct((Gamma_Y1*(gamma_Y.array().square().matrix())).transpose(), gamma_Y.transpose()*Theta + gamma_z.transpose()))*S_q_tilde*(diag_mu_4*phi.transpose())).transpose();
