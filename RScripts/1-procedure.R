...
make_recessions <- function(col="#80808044"){
...
make.model.implied.plot <- function(date,
  ...
  if(area.var <= 1){
    plot.fit.survey.distribution.mixture(data1,x1,param1,
                                         ...
                                         cex = .9)
    if(display.implied){
      lines(PDF.x.all[area.var,], PDF.all[time,,horizon.y,area.var], col="red",
            lwd=2)
    }
  ...
  if(display.implied & !display.mixture){
    ...
    if(display.legend){
      legend("bottom",inset = 0, title="Variables:", lty=c('blank','solid'),
             ...
             col = c("grey", "red"), fill=c("grey", "white"), border = c("grey", "white"),
             cex=0.65, horiz = TRUE)
    }
  }
