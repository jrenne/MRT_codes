...
for(i in 1:Model.final$m){
  ...
  plot(as.Date(vec.dates$date),KF.result4$xi.tT[,i], type ="l",
       col=color, lwd=2,
       xlab="",ylab="",las=1,
       main=main.t)
  grid()
  ...
  if(i==1){
    legend("bottomright",
           ...
           fill=c(color_demand, color_supply), cex=0.9, horiz = TRUE)
  }
}
...
plot(as.Date(vec.dates$date),z_ps, type ="l",
     col=color_supply, lwd=2,
     xlab="",ylab="",las=1,
     ylim=ylim,
     main=main.t)
lines(as.Date(vec.dates$date),z_ns, type ="l",
      col=color_supply, lwd=2,lty=3)
...
plot(as.Date(vec.dates$date),z_pd, type ="l",
     col=color_demand, lwd=2,
     xlab="",ylab="",las=1,
     ylim=ylim,
     main=main.t)
lines(as.Date(vec.dates$date),z_nd, type ="l",
      col=color_demand, lwd=2,lty=3)
grid()
make_recessions()
...
plot(as.Date(vec.dates$date),z_v, type ="l",
     col="black", lwd=2,
     xlab="",ylab="",las=1,
     main=main.t)
grid()
...
legend("topleft",
       c(expression(paste(z[p*','*t]^s,", positive component of the supply factor ",Y[1*','*t],sep='')),
         ...
         expression(paste(z[p*','*t]^d,", positive component of the supply factor ",Y[2*','*t],sep='')),
         expression(paste(z[n*','*t]^d,", negative component of the supply factor ",Y[2*','*t],sep='')),
